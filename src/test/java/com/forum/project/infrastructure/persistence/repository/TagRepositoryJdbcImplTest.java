...
class TagRepositoryJdbcImplTest {
    ...
    private void verifyTimestamps(Map<String, Object> result, Timestamp expectedTimestamp) {
        assertThat((Timestamp) result.get(TagKey.CREATED_DATE)).isAfter(expectedTimestamp);
        assertThat((Timestamp) result.get(TagKey.LAST_MODIFIED_DATE)).isAfter(expectedTimestamp);
    }
    ...
    void insertAndReturnGeneratedKeys() {
        ...
        verifyTimestamps(result, expectedTimestamp);
    }
    ...
    void saveAll() {
        ...
        result.forEach(r -> verifyTimestamps(r, expectedTimestamp));
    }
