...
import com.forum.project.common.utils.StringUtils;
...
public class QuestionQueries {
    ...
    public static String updateColumnsById(List<String> columns) {
        String setClause = columns.stream()
                .map(col -> col + " = :" + StringUtils.toCamelCase(col))
                .collect(Collectors.joining(", "));
        return "UPDATE questions SET " + setClause + " WHERE id =:id";
    }
}
